...
import {IBeefyVaultWithdraw} from '@interfaces/external/IStrategyStepExternal.sol';

/**
 * @title BeefyVaultWithdrawalStep
 * @notice Withdraws Beefy vault shares into the underlying LP token
 */
contract BeefyVaultWithdrawalStep is IStrategyStep {
  // --- Errors ---

  error BeefyVaultWithdrawalStep_InsufficientOutput();
  ...
  bytes32 internal constant _STEP_TYPE = bytes32('BEEFY_WITHDRAW');
  ...
  function preview(bytes calldata _data, uint256 _amountIn) external view returns (uint256[] memory _amountsOut) {
    ...
    _amountsOut[0] = (_amountIn * IBeefyVaultWithdraw(_decoded.vault).getPricePerFullShare()) / _scale;
  }
  ...
  function execute(
    ...
    IBeefyVaultWithdraw(_decoded.vault).withdraw(_amountIn);
    uint256 _after = IERC20(_decoded.lpToken).balanceOf(address(this));
    _amountsOut[0] = _after - _before;
    if (_minOuts.length > 0 && _amountsOut[0] < _minOuts[0]) revert BeefyVaultWithdrawalStep_InsufficientOutput();
  }
}
