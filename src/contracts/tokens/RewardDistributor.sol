    ...
    IRewardDistributor
{
    ...
    /// @inheritdoc IRewardDistributor
    mapping(bytes32 _root => mapping(address _account => bool _hasClaimed))
        public isClaimed;
    ...
    function updateMerkleRoots(
        ...
        for (uint256 _i = 0; _i < _tokens.length; _i++) {
            ...
            emit RewardDistributorMerkleRootUpdated(
                _tokens[_i],
                _merkleRoots[_i],
                epochCounter
            );
    ...
    function multiClaim(
        ...
        if (
            _tokens.length != _wads.length ||
            _wads.length != _merkleProofs.length
        ) {
            revert RewardDistributor_ArrayLengthsMustMatch();
        }
    ...
    function emergencyWithdraw(
        ...
        emit RewardDistributorEmergencyWithdrawal(
            _rescueReceiver,
            _token,
            _wad
        );
    ...
    function _claim(
        ...
        bytes32 _leaf = keccak256(
            bytes.concat(keccak256(abi.encode(address(msg.sender), _wad)))
        );

        if (MerkleProof.verify(_merkleProof, merkleRoots[_token], _leaf)) {
            bool success = IERC20(_token).transfer(msg.sender, _wad);
            if (!success) revert RewardDistributor_TransferFailed();
