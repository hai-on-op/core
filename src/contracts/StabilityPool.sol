...
contract StabilityPool is ERC4626, Authorizable, IStabilityPool {
  ...
  function coverAndRepayDebt(
    ...
    (uint256 _collateralBought, uint256 _adjustedBid) = _auction.getCollateralBought(_auctionId, _bidAmount);
    ...
    uint256 _collateralUsdValue = (_collateralBought * _collateralPriceUsd) / WAD;
    ...
    uint256 _estimatedHaiReceived = _strategy.estimateSwapToHai(_collateralType, _collateralBought);
    uint256 _estimatedHaiUsdValue = (_estimatedHaiReceived * _redemptionPrice) / RAY;
    ...
    uint256 _haiReceived = _strategy.swapToHai(_collateralType, _actualCollateralBought);
   ...
   */
  function setSwapStrategy(bytes32 _collateralType, ISwapStrategy _strategy) external isAuthorized {
    if (address(_strategy) == address(0)) {
  ...
  function _withdraw(
    ...
    uint256 _claimableKite = (shares * (kiteRewardIntegral - _userIntegral)) / WAD;
