...
contract Unit_PessimisticVeloSingleOracle_GetTwapPrice is PessimisticVeloSingleOracleTest {
  ...
  function test_GetCurrentPoolPrice_VolatileSingleFeedToken0CapRejectsTransientTrustedReserveRotation() public {
    token0Feed = new ChainlinkOracleForTest(8, 100_000_000, block.timestamp);
    ...
    // A volatile pool can naturally drift by more than the stable-pool 5% band against a multi-hour TWAP.
    // This should keep pricing live.
    pool.setReserves(900e6, 1.1e18);
    assertEq(oracle.getCurrentPoolPrice(false), 1800e8);

    // A one-block reserve rotation that removes almost all externally priced collateral must not be committed as
    // a delayed/pessimistic low. It is invalid until the TWAP window reflects the same reserve ratio.
    pool.setReserves(10e6, 100e18);
    vm.expectRevert(PessimisticVeloSingleOracle.SingleFeedReserveRatioDeviation.selector);
    oracle.getCurrentPoolPrice(false);

    pool.setConstantObservations(10e6, 100e18, POINTS + 1);
    assertApproxEqAbs(oracle.getCurrentPoolPrice(false), 20e8, 1);
  }

  function test_GetCurrentPoolPrice_VolatileSingleFeedToken1CapRejectsTransientTrustedReserveRotation() public {
    ...
    pool.setConstantObservations(100e18, 10e6, POINTS + 1);
    assertApproxEqAbs(oracle.getCurrentPoolPrice(false), 20e8, 1);
  }
  ...
  function test_GetCurrentPoolPrice_PessimisticRechecksStableDepegForSingleFeedViaLiveCap() public {
    // The pessimistic read path caps by the live fair price. For single-feed stable pools this means an
    // out-of-band TWAP-derived price fails closed instead of serving a stale cached low.
    token0Feed = new ChainlinkOracleForTest(8, 100_000_000, block.timestamp);
    pool = new VeloPoolForTest(token0, token1, true, 1e18, 1e18);
    ...
    // Skew the pool so the TWAP-derived unfed price is now far off-band.
  ...
  function test_UpdatePrice_StoresTrueObservedVolatileSingleFeedDailyLowAfterCrash() public {
    _deployOracle(false, 1e18, 1e18);
    _mockSequencerUp();
    oracle.setOperator(address(this), true);
    ...
    uint256 livePrice = oracle.getCurrentPoolPrice(false);
    oracle.updatePrice();

    assertEq(oracle.dailyUpdates(day), 2);
    assertEq(oracle.dailyLow(day), livePrice);
    assertApproxEqAbs(oracle.dailyLow(day), 200_000_000, 1);
    assertEq(oracle.lastPriceUpdateTime(), block.timestamp);
  }

  function test_UpdatePrice_RevertsBeforeRecordingTransientSingleFeedReserveRotation() public {
    token0Feed = new ChainlinkOracleForTest(8, 100_000_000, block.timestamp);
    pool = new VeloPoolForTest(token0, token1, false, 1e6, 1e18);
    pool.setConstantObservations(1000e6, 1e18, POINTS + 1);
    oracle = new PessimisticVeloSingleOracle(
    ...
    oracle.updatePrice();
    uint256 day = oracle.currentDay();
    uint256 cachedLow = oracle.dailyLow(day);
    ...
    assertEq(oracle.dailyUpdates(day), 1);
    assertEq(oracle.dailyLow(day), cachedLow);
  }

  function test_GetCurrentPoolPrice_PessimisticCapsStoredLowByLiveFairPrice() public {
    _deployOracle(false, 1e18, 1e18);
    _mockSequencerUp();
    oracle.setOperator(address(this), true);
    ...
    assertEq(cachedLow, 400_000_000);
    assertLt(livePrice, cachedLow);
    assertEq(oracle.getCurrentPoolPrice(true), livePrice);
    assertEq(oracle.dailyLow(day), cachedLow);
  }

  function test_UpdatePrice_RecordsVolatileSingleFeedDailyLowDrop() public {
    _deployOracle(false, 1e18, 1e18);
    _mockSequencerUp();
    oracle.setOperator(address(this), true);
  ...
  function test_UpdatePrice_RecordsTrueObservedDualFeedDailyLowDrop() public {
    _deployOracleWithFeeds(false, 1e18, 1e18, 100_000_000, 100_000_000);
    _mockSequencerUp();
    oracle.setOperator(address(this), true);
  ...
  function test_UpdatePrice_RecordsTrueObservedStableDailyLowDrop() public {
    _deployOracleWithFeeds(true, 1e18, 1e18, 100_000_000, 100_000_000);
    _mockSequencerUp();
    oracle.setOperator(address(this), true);
  ...
  function test_StableLpPrice_AllowsDualFeedTimestampSkewWithinHeartbeat() public {
    _deployOracleWithFeeds(true, 1e18, 1e18, 100_000_000, 100_000_000);
    _mockSequencerUp();
    token1Feed.set(100_000_000, block.timestamp - 59 minutes);

    assertGt(oracle.getCurrentPoolPrice(false), 0);
  }

  function test_StableLpPrice_RevertsWhenDualFeedTimestampSkewExceedsShortestHeartbeat() public {
    token0Feed = new ChainlinkOracleForTest(8, 100_000_000, block.timestamp);
    token1Feed = new ChainlinkOracleForTest(8, 100_000_000, block.timestamp);
    pool = new VeloPoolForTest(token0, token1, true, 1e18, 1e18);
    pool.setConstantObservations(1e18, 1e18, POINTS + 1);
    ...
    _mockSequencerUp();
    token1Feed.set(100_000_000, block.timestamp - 23 hours);

    vm.expectRevert(PessimisticVeloSingleOracle.DualFeedTimestampSkew.selector);
    oracle.getCurrentPoolPrice(false);
  }

  function test_VolatileLpPrice_DoesNotApplyDualFeedTimestampSkewCheck() public {
    _deployOracleWithFeeds(false, 1e18, 1e18, 100_000_000, 100_000_000);
    _mockSequencerUp();
    token1Feed.set(100_000_000, block.timestamp - 59 minutes);
