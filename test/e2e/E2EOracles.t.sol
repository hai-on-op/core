...
import {
    ChainlinkRelayer,
    IBaseOracle
} from "@contracts/oracles/ChainlinkRelayer.sol";
import {UniV3Relayer} from "@contracts/oracles/UniV3Relayer.sol";

import {
    DenominatedOracle,
    IDenominatedOracle
} from "@contracts/oracles/DenominatedOracle.sol";
import {
    DelayedOracle,
    IDelayedOracle
} from "@contracts/oracles/DelayedOracle.sol";
...
import {
    YearnVeloVaultRelayer,
    IYearnVeloVaultRelayer,
    IYearnVault
} from "@contracts/oracles/YearnVeloVaultRelayer.sol";
import {
    CurveStableSwapNGRelayer,
    ICurveStableSwapNGRelayer
} from "@contracts/oracles/CurveStableSwapNGRelayer.sol";
...
contract E2EOracleSetup is HaiTest {
    ...
    uint256 WSTETH_USD_PRICE =
        CHAINLINK_WSTETH_ETH_PRICE.wmul(CHAINLINK_ETH_USD_PRICE_18_DECIMALS);
    ...
    function setUp() public {
        ...
        // --- Chainlink ---
        wethUsdPriceSource = new ChainlinkRelayer(
            OP_CHAINLINK_ETH_USD_FEED,
            OP_CHAINLINK_SEQUENCER_UPTIME_FEED,
            1 days
        );
        wstethEthPriceSource = new ChainlinkRelayer(
            OP_CHAINLINK_WSTETH_ETH_FEED,
            OP_CHAINLINK_SEQUENCER_UPTIME_FEED,
            1 days
        );
        ...
        // --- UniV3 ---
        wbtcWethPriceSource = new UniV3Relayer(
            UNISWAP_V3_FACTORY,
            OP_WBTC,
            OP_WETH,
            FEE_TIER,
        ...
        // --- Denominated ---
        wstethUsdPriceSource = new DenominatedOracle(
            wstethEthPriceSource,
            wethUsdPriceSource,
            false
        );
        wbtcUsdPriceSource = new DenominatedOracle(
            wbtcWethPriceSource,
            wethUsdPriceSource,
            false
        );
        ...
        haiBoldRelayerOracle = new CurveStableSwapNGRelayer(
            OP_CURVE_BOLD_HAI_POOL,
            CURVE_BOLD_HAI_BASE_INDEX,
            CURVE_BOLD_HAI_QUOTE_INDEX
        );
    }
    ...
    function test_ChainlinkOracle() public {
        assertEq(
            IChainlinkOracle(OP_CHAINLINK_ETH_USD_FEED).latestAnswer(),
            int256(CHAINLINK_ETH_USD_PRICE)
        );
    }
    ...
    function test_ChainlinkRelayer() public {
        ...
        assertEq(
            ...
            CHAINLINK_ETH_USD_PRICE_18_DECIMALS
        );
    }
    ...
    function test_DelayedOracle() public {
        assertEq(
            wethUsdDelayedOracle.read(),
            CHAINLINK_ETH_USD_PRICE_18_DECIMALS
        );

        (uint256 _result, bool _validity) = wethUsdDelayedOracle
            .getResultWithValidity();
        assertTrue(_validity);
        assertEq(_result, CHAINLINK_ETH_USD_PRICE_18_DECIMALS);

        (uint256 _nextResult, bool _nextValidity) = wethUsdDelayedOracle
    ...
    function test_DelayedOracleUpdateResult() public {
        ...
        (uint256 _nextResult, ) = wethUsdDelayedOracle
            .getNextResultWithValidity();
    ...
    function test_BeefyVeloVaultRelayerSymbol() public {
        assertEq(
            beefyVeloVaultRelayerOracle.symbol(),
            "mooVelodromeOptimismBOLD-LUSD / USD"
        );
    }
    ...
    function test_YearnVeloVaultRelayerSymbol() public {
        assertEq(
            yearnVeloVaultRelayerOracle.symbol(),
            "yvVelo-alETH-WETH-f / USD"
        );
    }
